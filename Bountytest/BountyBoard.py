...
class BountyBoard(commands.Cog):
    ...
    async def toggle_reward(self, ctx):
        current_value = await self.config.guild(ctx.guild).ask_reward()
        new_value = not current_value
        await self.config.guild(ctx.guild).ask_reward.set(new_value)
        await ctx.send(f"The reward question has been {'enabled' if new_value else 'disabled'}.")

    @bounty_group.command(name="toggle_supervisor")
    @checks.admin_or_permissions(manage_guild=True)
    async def toggle_supervisor(self, ctx):
        current_value = await self.config.guild(ctx.guild).ask_supervisor()
        new_value = not current_value
        await self.config.guild(ctx.guild).ask_supervisor.set(new_value)
        await ctx.send(f"The supervisor question has been {'enabled' if new_value else 'disabled'}.")
