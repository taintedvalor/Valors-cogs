...
class BountyBoard(commands.Cog):
            ...
            if bounty.name.lower() == bounty_name.lower():
                if not bounty.completed:
                    if ctx.author == bounty.poster or ctx.author == bounty.supervisor:
                        ...
                        # Give a star to the user who completed the bounty
    ...
    @bounty_group.command(name="toggle_reward")
    @checks.admin_or_permissions(manage_guild=True)
    async def toggle_reward(self, ctx):
        toggle_reward = await self.config.guild(ctx.guild).toggle_reward()
        await self.config.guild(ctx.guild).toggle_reward.set(not toggle_reward)
        status = "on" if not toggle_reward else "off"
        await ctx.send(f"The reward question has been toggled {status}.")

    @bounty_group.command(name="toggle_supervisor")
    @checks.admin_or_permissions(manage_guild=True)
    async def toggle_supervisor(self, ctx):
        toggle_supervisor = await self.config.guild(ctx.guild).toggle_supervisor()
        await self.config.guild(ctx.guild).toggle_supervisor.set(not toggle_supervisor)
        status = "on" if not toggle_supervisor else "off"
        await ctx.send(f"The supervisor question has been toggled {status}.")

    @bounty_group.command(name="stars")
    async def bounty_stars(self, ctx, user=None):
        if user is None:
            user = ctx.author

        star_count = self.get_star_count(user)
    ...
    def get_star_count(self, user):
        return self.stars.get(user.id, 0)
