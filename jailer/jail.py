...
class Jail(commands.Cog):
    ...
    async def jail(self, ctx, member: discord.Member):
        ...
        jail_role = await ctx.guild.create_role(name='Jail Role', permissions=discord.Permissions(send_messages=False, read_messages=False))
        await member.add_roles(jail_role, reason='Jailed')

        # Set the jail text channel permissions
        for role in ctx.guild.roles:
            if role.name in self.allowed_roles or role == jail_role:
                await jail_channel.set_permissions(role, read_messages=True)
