...
class RandImages(Core):
    ...
    @commands.cooldown(1, 0.5, commands.BucketType.user)
    @commands.bot_has_permissions(embed_links=True)
    @commands.command(aliases=["cunt"])
    async def pussy(self, ctx: commands.Context):
        """pussy."""

        await self._send_reddit_msg(
            ctx,
            name=_("heres some pussy"),
            emoji="",
            sub=sub.PUSSY,
            details=False,
        )
    ...
    async def fuck(self, ctx, user: commands.MemberConverter):
        ...
        await self._send_reddit_msg(
            ctx,
            name=(f"{ctx.author.name} fucked {user.name}!"),
            emoji="",
            sub=sub.FUCK,
            details=False,
        )
    ...
    @commands.cooldown(1, 0.5, commands.BucketType.user)
    @commands.bot_has_permissions(embed_links=True)
    @commands.command(aliases=["fporn"])
    async def foodporn(self, ctx: commands.Context):
        """foodporn."""
