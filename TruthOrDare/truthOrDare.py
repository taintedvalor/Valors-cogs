        ...
        while True:
            ...
            if str(reaction.emoji) == truth_emoji:
                ...
                if ctx.channel.is_nsfw():
                    ...
                    await message.delete()
                    nsfw_confirm_message = await ctx.send("This command contains NSFW content. Are you sure you want to continue?")
                    await nsfw_confirm_message.add_reaction("✅")  # Confirm
                    await nsfw_confirm_message.add_reaction("❌")  # Cancel

                    def nsfw_confirm_check(confirm_reaction, confirm_user):
                        return (
                            confirm_user == ctx.author
                            and confirm_reaction.message.id == nsfw_confirm_message.id
                            and str(confirm_reaction.emoji) in ["✅", "❌"]
                        )

                    try:
                        confirm_reaction, _ = await self.bot.wait_for(
                            "reaction_add", check=nsfw_confirm_check, timeout=30.0
                        )
                    except asyncio.TimeoutError:
                        await ctx.send("timed out origin messages removed.")
                        await nsfw_confirm_message.delete()
                        break
