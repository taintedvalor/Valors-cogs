...
class Promotion(commands.Cog):
    ...
    @promotion.command(name="demote")
    @commands.admin()
    async def demote(self, ctx, user: discord.Member, role: discord.Role):
        """
        Demotes a user by removing a specified role.

        Usage: !promotion demote [user] [role]
        """
        if role in user.roles:
            await user.remove_roles(role)
            await ctx.send(f"{user.name} has been demoted and the {role.name} role has been removed.")
        else:
            await ctx.send(f"{user.name} does not have the {role.name} role.")
