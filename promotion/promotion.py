...
class Promotion(commands.Cog):
    ...
    async def set_promotion_channel(self, ctx, channel: discord.TextChannel = None):
        ...
        Usage: !promotion set_promotion_channel [channel]
        """
    ...
    async def add_staff_role(self, ctx, role: discord.Role):
        ...
        Usage: !promotion add_staff_role [role]
        """
        guild_id = ctx.guild.id
    ...
    async def remove_staff_role(self, ctx, role: discord.Role):
        ...
        Usage: !promotion remove_staff_role [role]
        """
        guild_id = ctx.guild.id
        ...
        if guild_id in self.guild_settings:
            ...
            if promotion_channel_id is not None:
                ...
                if promotion_channel is not None:
                    ...
                    before_staff_role_ids = [role.id for role in before.roles]

                    # Check if a staff role has been applied
                    for staff_role_id in self.guild_settings[guild_id].get("staff_roles", []):
                        if staff_role_id in staff_role_ids and staff_role_id not in before_staff_role_ids:
                            blocks = ""
                            for c in after.display_name:
                                if c.isalpha():
                                    ...
                                    blocks += " "
                            await promotion_channel.send(f"{after.mention} has been promoted to {blocks}!")

                    # Check if a staff role has been removed
                    for staff_role_id in self.guild_settings[guild_id].get("staff_roles", []):
                        if staff_role_id in before_staff_role_ids and staff_role_id not in staff_role_ids:
                            blocks = ""
                            for c in after.display_name:
                                if c.isalpha():
                                    blocks += ":regional_indicator_{}: ".format(c).lower()
