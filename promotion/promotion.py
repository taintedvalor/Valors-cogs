...
class Promotion(commands.Cog):
    ...
    async def demote(self, ctx, user: discord.Member):
        staff_role_ids = self.guild_settings.get(ctx.guild.id, {}).get("staff_roles", [])
        roles_to_remove = [role for role in user.roles if role.id in staff_role_ids]

        if roles_to_remove:
            await user.remove_roles(*roles_to_remove)
            await ctx.send(f"All staff roles have been removed from {user.display_name}.")
        else:
            await ctx.send(f"{user.display_name} does not have any staff roles to remove.")

    @promotion.command()
    @commands.admin()
    async def list_staff_roles(self, ctx):
        guild_id = ctx.guild.id
        staff_roles = self.guild_settings.get(guild_id, {}).get("staff_roles", [])

        if staff_roles:
            role_mentions = [f"<@&{role_id}>" for role_id in staff_roles]
            await ctx.send(f"The following roles are configured as staff roles: {' '.join(role_mentions)}")
        else:
            ...
            if promotion_channel_id is not None:
                ...
                if promotion_channel is not None:
                    ...
                    for staff_role_id in self.guild_settings[guild_id].get("staff_roles", []):
                        if staff_role_id in staff_role_ids and staff_role_id not in before_role_ids:
                            blocks = ""
                        ...
                        if staff_role_id in before_role_ids and staff_role_id not in staff_role_ids:
                            blocks = ""
                            for c in after.display_name:
                                if c.isalpha():
                                    blocks += ":regional_indicator_{}: ".format(c).lower()
                                    ...
                                    blocks += " "
                            await promotion_channel.send(f"{after.mention} has been demoted from {blocks}.")
