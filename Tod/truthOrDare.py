...
class TruthOrDare(commands.Cog):
    ...
    async def truth_or_dare(self, ctx):
        ...
        truth_embed = discord.Embed(title="Random Truth Question", description=random.choice(messages["truth"]))
        truth_embed.set_author(name="Truth", icon_url="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/game-die_1f3b2.png")

        dare_embed = discord.Embed(title="Random Dare Question", description=random.choice(messages["dare"]))
        dare_embed.set_author(name="Dare", icon_url="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/target_1f3af.png")

        nsfw_embed = discord.Embed(title="Random NSFW Question", description="Click the NSFW button to reveal a question.")
        nsfw_embed.set_author(name="NSFW Question", icon_url="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/no-entry_26d4.png")
        ...
        while True:
            try:
                ...
                await ctx.send("You didn't click an emoji in time.")
                await message.delete()
                break

            if str(reaction.emoji) == truth_emoji:
                await ctx.send(embed=truth_embed)
            elif str(reaction.emoji) == dare_emoji:
                await ctx.send(embed=dare_embed)
                ...
                if ctx.channel.is_nsfw():
                    ...
                    while True:
                        ...
                        if str(nsfw_reaction.emoji) == nsfw_reactions[0]:
                            embed = discord.Embed(title="Random NSFW Truth Question", description=random.choice(messages["nsfw_truth"]))
                            await ctx.send(embed=embed)
                        elif str(nsfw_reaction.emoji) == nsfw_reactions[1]:
                            embed = discord.Embed(title="Random NSFW Dare Question", description=random.choice(messages["nsfw_dare"]))
                            await ctx.send(embed=embed)
                else:
