...
class TruthOrDare(commands.Cog):
    ...
    async def truth_or_dare(self, ctx):
        ...
        message = await ctx.send("Click an emoji to get a random question:\n"
                                 "🎲 - Truth\n"
                                 "🎯 - Dare\n"
                                 "🔞 - NSFW Question")
        truth_emoji = "🎲"
        ...
        while True:
            ...
            if str(reaction.emoji) == truth_emoji:
                ...
                embed = discord.Embed(title="Random Truth Question", description=random_message)
                await ctx.send(embed=embed)
            elif str(reaction.emoji) == dare_emoji:
                random_message = random.choice(messages["dare"])
                embed = discord.Embed(title="Random Dare Question", description=random_message)
                await ctx.send(embed=embed)
            elif str(reaction.emoji) == nsfw_emoji:
                if ctx.channel.is_nsfw():
                    nsfw_reactions = ["💣", "💥"]  # NSFW reactions to swap to
                    nsfw_message = await ctx.send("Click an emoji to get a random NSFW question:\n"
                                                  "💣 - NSFW Truth\n"
                                                  "💥 - NSFW Dare")
                    ...
                    while True:
                        ...
                        if str(nsfw_reaction.emoji) == nsfw_reactions[0]:
                            ...
                            embed = discord.Embed(title="Random NSFW Truth Question", description=random_message)
                            await ctx.send(embed=embed)
                        elif str(nsfw_reaction.emoji) == nsfw_reactions[1]:
                            random_message = random.choice(messages["nsfw_dare"])
                            embed = discord.Embed(title="Random NSFW Dare Question", description=random_message)
                            await ctx.send(embed=embed)
                else:
