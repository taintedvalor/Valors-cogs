...
class BountyBoard(commands.Cog):
    ...
    async def create_bounty(self, ctx, name, *, description, reward=None, supervisor=False):
        bounty = Bounty(name, description, reward)
        bounty.poster = ctx.author

        if supervisor:
            supervisor_role = ctx.guild.default_role
            for role in ctx.guild.roles:
                if role.permissions.administrator:
                    supervisor_role = role
                    break

            bounty.supervisor = supervisor_role
            await ctx.send(f"Supervisor role {supervisor_role.mention} assigned to bounty '{bounty.name}'!")

            # Notify the supervisor
            supervisor_member = ctx.guild.get_member(supervisor_role.id)
            if supervisor_member:
                await supervisor_member.send(f"You have been assigned as the supervisor for bounty '{bounty.name}'!")
