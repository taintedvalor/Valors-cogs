...
class BountyBoard(commands.Cog):
    ...
    async def set_supervisor(self, ctx, bounty_id: int, supervisor: commands.RoleConverter):
        if bounty_id < 0 or bounty_id >= len(self.bounties):
            await ctx.send("Invalid bounty ID!")
            return
    ...
    @commands.has_permissions(administrator=True)
    async def configure_supervisor_role(self, ctx, role: commands.RoleConverter):
